...
public abstract class Level extends SaveableEntity {
  ...
  public void update(float dt) {
    super.update(dt);    if (this != Dungeon.level) {
      ...
      for (int y = 0; y < getHeight(); y++) {
        if (this.checkFor(x, y, Terrain.SOLID)) {
          ...
          if (total < 8) {
            ...
            int xx = x * 16;
            int yy = y * 16;            if (this.checkFor(x, y + 1, Terrain.SOLID)) {
          ...
          if (total < 8) {
            PolygonShape poly = new PolygonShape();
            int xx = x * 16;
            int yy = y * 16;            if (this.checkFor(x, y + 1, Terrain.HOLE)) {
              ArrayList<Vector2> array = new ArrayList<>();
