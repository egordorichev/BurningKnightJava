...
public class GoldenChest extends Chest {
	...
	public Item generate() {
		...
		for (ItemRegistry.Pair item : ItemRegistry.INSTANCE.getItems().values()) {
			if (item.getQuality() == ItemRegistry.Quality.GOLDEN && Achievements.unlocked(item.getUnlock()) && (weapon ? WeaponBase.class : Accessory.class).isAssignableFrom(item.getType()) && Player.instance.getInventory().findItem(item.getType()) == null) {
