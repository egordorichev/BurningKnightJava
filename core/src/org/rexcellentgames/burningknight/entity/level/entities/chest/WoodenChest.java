...
public class WoodenChest extends Chest {
	...
	public Item generate() {
		...
		for (ItemRegistry.Pair item : ItemRegistry.INSTANCE.getItems().values()) {
			if (item.getQuality() == ItemRegistry.Quality.WOODEN && Achievements.unlocked(item.getUnlock()) && (weapon ? WeaponBase.class : Accessory.class).isAssignableFrom(item.getType())) {
