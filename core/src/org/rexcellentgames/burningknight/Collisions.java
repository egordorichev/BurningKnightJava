...
public class Collisions implements ContactListener, ContactFilter {
	...
	public void preSolve(Contact contact, Manifold oldManifold) {
		...
		if (a == null && contact.getFixtureA().getBody().isBullet() && b instanceof Player && ((Player) b).flying) {
		...
		} else if (b instanceof Door && (a instanceof Creature || a instanceof PetEntity)) {
			if (!((Door) b).lock || a instanceof PetEntity) {
				contact.setEnabled(false);
			}
			...
			contact.setEnabled(false);
		} else if (a instanceof HeartFx && !(b instanceof HeartFx || b instanceof SolidProp) && b != null) {
			contact.setEnabled(false);
		} else if (b instanceof HeartFx && !(a instanceof HeartFx || a instanceof SolidProp) && a != null) {
			contact.setEnabled(false);
